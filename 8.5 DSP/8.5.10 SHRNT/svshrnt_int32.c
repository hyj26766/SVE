...
static void calc_vecshrnb_opt(HalfScalar *r,HalfScalar *even,ScalarType *op1,size_t cmputSize)
{
    ...
    for (size_t i=0;i<cmputSize;i+=COUNT())
    {
        ...
        HalfScalar sveven=svld1(pred,even+(i*2))
        //result
        HalfScalar svr=svshrnt(svop1,imm);
        //Store result
        svst1(pred1,r+(i*2),svr);

    } 
}
static void calc_vecshrnt_ref(HalfScalar *out,HalfScalar *even,ScalarType *op1,size_t cmputSize)
{
...
int test_svshrnb_int16(size_t cmputSize)
{
    ...
    ScalarType *even=(ScalarType*)malloc(cmputSize*sizeof(ScalarType));
    ...
    calc_vecshrnb_opt((HalfScalar*)opt_x,(HalfScalar*)even,op1,cmputSize);
    calc_vecshrnb_ref((HalfScalar*)ref_x,(HalfScalar*)even,op1,cmputSize);
