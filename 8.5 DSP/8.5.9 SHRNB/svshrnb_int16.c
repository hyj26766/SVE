...
static void calc_vecshrnb_opt(ScalarType *r,ScalarType *op1,ScalarType *op2,size_t cmputSize)
{
    ...
    for (size_t i=0;i<cmputSize;i+=COUNT())
    {
        ...
        //result
        VectorType svr=svshrnb(svop1,svop2,randimm);
        //Store result
        svst1(pred1,r+i,svr);

    } 
}
static void calc_vecshrnb_ref(ScalarType *out,ScalarType *op1,ScalarType *op2,size_t cmputSize)
{
    for (size_t i=0;i<cmputSize;++i)
    {
        ScalarType temp1=op1[i]-(op1[i]>>imm3<<imm3);//取op1后imm3位
        Ustype temp2=((Ustype)op2[i]<<imm3)+temp1;
        out[i]=(ScalarType)temp2;
}
}
...
int test_svshrnb_int16(size_t cmputSize)
{
    ...
    calc_vecshrnb_opt(opt_x,op1,op2,cmputSize);
    calc_vecshrnb_ref(ref_x,op1,op2,cmputSize);
