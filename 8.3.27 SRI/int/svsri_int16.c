...
static void calc_vecsri_opt(ScalarType *r,ScalarType *op1,ScalarType *op2,size_t cmputSize)
{
    ...
    for (size_t i=0;i<cmputSize;i+=COUNT())
    {
        ...
        //result
        VectorType svr=svsri(svop1,svop2,imm3);
        //Store result
        svst1(pred1,r+i,svr);

    } 
}
static void calc_vecsri_ref(ScalarType *out,ScalarType *op1,ScalarType *op2,size_t cmputSize)
{
    for (size_t i=0;i<cmputSize;++i)
    {
        Ustype tempop2=((Ustype)op2[i])>>imm3;//取op2后左l-imm3位作为低位
        out[i]=(op1[i]>>(Bitsofs-imm3)<<(Bitsofs-imm3))+((ScalarType)tempop2);
        
        
}
}

int test_svsri_int16(size_t cmputSize)
{
    ...
    for (size_t i=0;i<cmputSize;++i)
    {
        ref_x[i]=bigrand()%((Doublelenth)2<<Bitsofs-1)+MIN_VALUE;
        opt_x[i]=bigrand()%((Doublelenth)2<<Bitsofs-1)+MIN_VALUE;
        op1[i]=bigrand()%((Doublelenth)2<<Bitsofs-1)+MIN_VALUE;
        op2[i]=bigrand()%((Doublelenth)2<<Bitsofs-1)+MIN_VALUE;
    }

    calc_vecsri_opt(opt_x,op1,op2,cmputSize);
    calc_vecsri_ref(ref_x,op1,op2,cmputSize);
    ...
    for (size_t i=0;i<cmputSize;++i)
    {
        if(ref_x[i]!=opt_x[i])
        {
            ...
            printf("ERROR:%lu,op1=%lld,op2=%lld,imm3=%llu,ref_x=%lld,opt_x=%lld\n",i,(__int64_t)op1[i],(__int64_t)op2[i],imm3,(__int64_t)ref_x[i],(__int64_t)opt_x[i]);
            ret=1;
