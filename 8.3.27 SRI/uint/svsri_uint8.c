...
#define ScalarType uint8_t
#define VectorType svuint8_t
//#define Ustype uint8_t
#define Doublelenth __uint16_t
...
static void calc_vecsri_ref(ScalarType *out,ScalarType *op1,ScalarType *op2,size_t cmputSize)
{
    ...
    {
        ScalarType tempop2=op2[i]>>imm3;//取op2后左l-imm3位作为低位
        out[i]=(op1[i]>>(Bitsofs-imm3)<<(Bitsofs-imm3))+tempop2;
}
}
